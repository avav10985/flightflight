...
void handleRoot() {
  String html =
    ...
    "<style>"
    "body{font-family:sans-serif;margin:20px;background:#222;color:#0f0;}"
    "button{font-size:24px;padding:12px 24px;margin:10px 0;}"
    "h1{font-size:22px;}"
    "p{font-size:16px;}"
    ".v{font-size:32px;font-family:monospace;color:#ff0;}"
    "</style></head><body>"
    "<h1>頭追原型(手機 IMU → 飛機 servo)</h1>"
    "<button onclick='start()'>授權 + 開始</button>"
    "<p>α(yaw 指南針):<span class='v' id='a'>--</span></p>"
    "<p>β(pitch 前後):<span class='v' id='b'>--</span></p>"
    "<p>γ(roll 左右):<span class='v' id='g'>--</span></p>"
    "<p id='status'>未開始</p>"
    "<script>"
    "let lastSend = 0;"
    ...
    "    if (p !== 'granted') { document.getElementById('status').innerText = '權限被拒';return; }"
    "  }"
    "  document.getElementById('status').innerText = '運作中,每 50ms 送一次資料';"
    ...
    "    if (now - lastSend > 50) {"
    "      lastSend = now;"
    "      fetch(`/o?a=${e.alpha}&b=${e.beta}&g=${e.gamma}`).catch(()=>{});"
    "    }"
    "  });"
    "}"
    "</script></body></html>";
